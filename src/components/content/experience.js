...
const employments = [
	...
	},
	{
		...
		"worked on in house projects such as maintenanc eof a patient management system for dental care built with ASP.NET and assisting in R&D for VR/AR applications."
	}
]
...
export default class Experience extends React.Component {
	render() {
		const employtmentItems = employments.map((employment, index) => {
			return(
				<div className={`experience_content experienceSection experience_content-${index % 2 === 0 ? "left" : "right"}`}>
		...
		const educationItems = educations.map((edication, index) => {
			return(
				<div className={`experience_content experienceSection experience_content-${index % educationsIndexing === 0 ? "left" : "right"}`}>
					<div className="experienceSection_header">
						<div>
							{edication.location}
						</div>
		...
		const projectItems = projects.map((project, index) => {
			return(
				<div className={`experience_content experienceSection experience_content-${index % projectsIndexing === 0 ? "left" : "right"}`}>
					<div className="experienceSection_header">
						<div>
							{project.location}
						</div>
